...
# (a) Using the R function arima.sim(), simulate and plot 250 observations of the MA(1), 
#     theoretical ACF (autocorrelation function) and sample ACF with =0.5, =0.9 and =-0.9.
...
# The MA(1) time series shows a spike of time dependence at one lag for this simulation. The direction 
# depends on the sign of theta. Note that this spike is outside the confidence interval. Also, the higher
# theta can can result in a lower or higher than usual first point in the series.


## 1. Consider the AR(1) model

# (a) Using the R function arima.sim(), simulate and plot 250 observations of the AR(1) 
#  with =0, =0.5, =0.9 and =0.99.
...
# (b) Comment on the behavior of the simulated data series. Which series is close to 
#  nonstationary (or persistent) time series?
# The AR(1) time series shows time dependence that decays slowly over time. We can observe that the higher
# phi associates with stronger persistence. However, when it gets too close to 1 it becomes like
...
# VBLTX is the Vanguard Long-Term Bond Index mutual fund, FMAGX is the Fidelity Magellan Fund, and SBUX
# is Starbucks stock.
...
# (a) Make time plots of the returns. Comment on any relationships between the returns suggested by the plots. 
#     Pay particular attention to the behavior of returns toward the end of 2008 at the beginning of the financial crisis.
chart.TimeSeries(lab4Returns.z, legend.loc="bottom", main="") 

# All time series appear to be stationary with varying levels of volatility. There is more volatility towards the financial
# crisis, especially in SBUX and FMAGX. 
...
# From this plot, the VBLTX gave the best future value of the investment at present time. But we also note that Starbucks
# had the highest value around 3.5, before it dropped significantly in 2007. The FMAGX consistently had the worst 
# future values.

# (c) For each return series, make a four panel plot containing a histogram, density plot, boxplot and normal QQ-plot. 
#     Do the return series look normally distributed? Briefly compare the return distributions.
...
# For the most part, the returns of these time series all look somewhat normal. The QQ-Plots all show fatter tails
# than a normal distribution, which is expected in financial return data. We can also see that Starbucks has
# a slightly higher median value than zero.


# (d) Compute numerical descriptive statistics for all assets using the R functions summary(), mean(), var(), 
#     stdev(), skewness(), and kurtosis() (in package PerformanceAnalytics). 
#     Compare and contrast the descriptive statistics for the three assets. Which asset appears to be the riskiest asset?
table.Stats(lab4Returns.z)

# Here we again see that SBUX has a median return of about 2% while the others are around 0.1%. Starbucks also has
# the highest maximum value. However, Starbuck's variance is at 0.01 while the other series are 0.0007 and 0.0041. All series
# have a slight negative skew, meaning longer tails to the left than the right. Starbucks and VBLTX have lower than normal
# Kurtosis and FMAGX has a higher kurtosis. From these results we can see that Starbucks is the riskiest asset, but also
# has the chance for highest return.


# (e) Using the mean monthly return for each asset, compute an estimate of the annual continuously compounded
#     return (i.e., recall the relationship between the expected monthly cc return and the expected annual cc return). 
#     Convert this annual continuously compounded return into a simple annual return. Are there any surprises?
...
# Here we see Starbucks has the highest cc return at 0.14, with VBLTX in second at 0.06 and FMAGX at 0.02. When
# we convert these to simple returns all the values are slightly higher.


# (f) Using the estimate of the monthly return standard deviation for each asset, compute an estimate of the 
#     annual return standard deviation. Briefly comment on the magnitude of the annual standard deviations.
...
# (g) Use the R pairs() function to create all pair-wise scatterplots of returns. 
#     Comment on the direction and strength of the linear relationships in these plots.
pairs(ret.mat, col="slateblue1", pch=16)
# From our plot we can see that FMAGX and SBUX have a positive association. FMAGX and VBLTX also appear to have
# weak positive associations. VBLTX and SBUX seem to have a negative association. All of these relationships 
# look somewhat weak, with the exception of VBLTX and SBUX.

# (h) Use the R functions var() and cor() to compute the sample covariance matrix and sample correlation 
#     matrix of the returns. Comment on the direction and strength of the linear relationships suggested 
#     by the values of the covariances and correlations.
var(ret.mat)
cor(ret.mat)
# The matrix tells us that FMAGX and VBLTX have a weak positive correlation, VBLTX and SBUX have a weak
# negative correlation, and SBUX and FMAGX have a moderate positive correlation.

# (i) Use the R function acf() to compute and plot the sample autocorrelation functions of each return. 
#     Do the returns appear to be uncorrelated over time?
par(mfrow=c(3,1))
...
# The ACF plots tell us that all the series are not time dependent. VBLTX has one significat spike at lag 2
# but this probably does not carry any economic meaning and is due to chance.
  
  
## 2. (IID Normal Model) Consider the IID normal model
# where Rit denotes the continuously compounded return on asset i, i =Vanguard long term bond index fund (VBLTX), 
# Fidelity Magellan stock mutual fund (FMAGX), and Starbucks stock (SBUX).
...
# Our estimates of these parameters all coincide with what we observed earlier. SBUX has the highest mean value
# but also the largest variance(risk). The covariane and correlation coefficients also indicate the same relationships
# as noted earlier.


# (b) For each estimate of the above parameters (except   ) compute ij the estimated standard error. 
#     That is, compute SE ( ^ ), SE  ^2, SE ( ^i) and SE  ^ij . Briefly comment on the precision of the estimates.
nobs = nrow(ret.mat)
...
# (c) For each parameter compute 95% and 99% confidence intervals. 
...
# These intervals for the top 2 are somewhat wide and between negative and positive numbers. The bottom
# row looks narrower and on the same side of zero.

# (d) Compute the 1% and 5% monthly value-at-Risk 
#     (VaR) based on an initial $100,000 investment. Which fund has the lowest VaR?
